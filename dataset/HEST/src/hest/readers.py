...
def calculate_centroid(polygon_coords):
    ...
    Args:
        polygon_coords (list of tuple): List of (x, y) tuples representing polygon vertices.
...
def pool_cell_transcripts_xenium(
    df: Union[pd.DataFrame, dd.DataFrame], 
    pixel_size_he: float,
    shapes,
    key_x='he_x',
    ...
    features = df['feature_name'].unique()
    #shapes = df.get_shapes('xenium_cell', 'he').shapes
    if isinstance(df, dd.DataFrame):
        features = features.compute()
        cell_list = cell_list.compute()
    
    spot_grid = pd.DataFrame(0, index=cell_list, columns=features)
    ...
    # extract nuc only seq
    #df = df[df['overlaps_nucleus']==1]
    features = df['feature_name']
    cell_to_index = {cell: idx for idx, cell in enumerate(cell_list)}
    index_list = [cell_to_index[inst] for inst in df['cell_id'].values]
    
    cols = spot_grid.columns.get_indexer(features)
    
    ## use dask for this parts
    spot_grid_np = spot_grid.values.astype(np.uint16)
    np.add.at(spot_grid_np, (index_list, cols), 1)
    
    index_map = {value: idx for idx, value in enumerate(shapes.index)}
    result_idx = [index_map[x] for x in cell_list if x in index_map]
    centorid_cells = [calculate_centroid(g[0]) for g in shapes.values[result_idx]]
...
#        y_min = y_min.compute()
    ...
    adata.var_names = adata.var_names.astype(str)
    #adata.obsm['spatial'] = coord_df[['x', 'y']].values
    adata.obsm['spatial'] = np.array(centorid_cells)
    adata.obs['in_tissue'] = True
    #adata.obs['pxl_col_in_fullres'] = coord_df['x'].values
    #adata.obs['pxl_row_in_fullres'] = coord_df['y'].values
    #adata.obs['array_col'] = np.arange(len(adata.obs)) % n
