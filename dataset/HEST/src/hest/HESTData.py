...
def compute_missing_patches_fixed_grid(image_shape, patch_size, existing_coords):

    # 用第一个 patch 坐标推断偏移
    x0, y0 = existing_coords[0]
    x_offset = x0 % patch_size
    y_offset = y0 % patch_size

    H, W = image_shape  # height, width

    # 生成 aligned 网格
    xs = np.arange(x_offset, W - patch_size, patch_size)
    ys = np.arange(y_offset, H - patch_size, patch_size)
    full_grid = [(x, y) for y in ys for x in xs]
    existing_set = set((int(x), int(y)) for x, y in existing_coords)
    missing_coords = [list(coord) for coord in full_grid if coord not in existing_set]
...
def compute_iou_total(candidate, existing_rects):
    inter_total = 0
    for r in existing_rects:
        inter_total += rect_intersection_area(candidate, r)

    area_cand = candidate[2] * candidate[3]
    area_exist = len(existing_rects) * (candidate[2] * candidate[3])  # All same size
    union = area_cand + area_exist - inter_total
    if union == 0:
        return 0.0
    return inter_total / union

def compute_remaining_patches_iou_total(image_shape, patch_size, existing_coords, stride=None, iou_thresh=0.5):
    if stride is None:
        stride = patch_size
    ...
    W, H = image_shape
    existing_rects = [(x, y, patch_size, patch_size) for x, y in existing_coords]

    remaining = []
    for y in range(y_offset, H - patch_size + 1, stride):
        for x in range(x_offset, W - patch_size + 1, stride):
            cand = (x, y, patch_size, patch_size)
            iou_total = compute_iou_total(cand, existing_rects)
            if iou_total <= iou_thresh:
...
class HESTData:
    ...
    def dump_patches(
        ...
        return patcher, valid_barcodes

    def dump_noseq_patches(
        self,
        patch_save_dir: str,
        name: str = 'patches',
        target_patch_size: int=224,
        ...
        in_slide_mask = (0 <= coords_topleft[:, 0] + patch_size_src) & (coords_topleft[:, 0] < self.wsi.width) & (0 <= coords_topleft[:, 1] + patch_size_src) & (coords_topleft[:, 1] < self.wsi.height)
        coords_topleft = coords_topleft[in_slide_mask]
        before_coords_topleft = np.array(coords_topleft).astype(int)
        #coords_topleft = compute_missing_patches_fixed_grid([self.wsi.height, self.wsi.width], target_patch_size, before_coords_topleft)
        coords_topleft = compute_remaining_patches_iou_total( image_shape=(self.wsi.width, self.wsi.height),patch_size=224, existing_coords=before_coords_topleft, stride=stride, iou_thresh=iou_thresh)
        coords_topleft = np.array(coords_topleft).astype(int)
        #if len(coords_topleft) < len_tmp:
        #    warnings.warn(f"Filtered {len_tmp - len(coords_topleft)} spots outside the WSI")
        ...
        #patcher.to_h5(h5_path, extra_assets={'barcode': valid_barcodes})
        coords_topleft = coords_topleft[patcher.valid_mask]
        if dump_visualization:
            patcher.save_visualization(os.path.join(patch_save_dir, name + '_noseq_patch_vis.png'), dpi=400)
            
        if verbose:
            print(f'found {patch_count} valid patches')
        center_coords = coords_topleft + patch_size_src//2
        return patcher, center_coords, before_coords_topleft
    
    def dump_noseq_cell_patches(
        self,
        patch_save_dir: str,
        name: str = 'patches',
        target_patch_size: int=224,
        ...
        y_min = np.min(adata.obsm['spatial'][:,1])
        y_max = np.max(adata.obsm['spatial'][:,1])
        used_rect = box(x_min, y_min, x_max, y_max)
        outside_points = [pt for pt in centroid_list if not used_rect.contains(Point(pt))]
        coords_topleft = np.array(outside_points) - patch_size_src // 2
        #len_tmp = len(coords_topleft)
        in_slide_mask = (0 <= coords_topleft[:, 0] + patch_size_src) & (coords_topleft[:, 0] < self.wsi.width) & (0 <= coords_topleft[:, 1] + patch_size_src) & (coords_topleft[:, 1] < self.wsi.height)
        coords_topleft = coords_topleft[in_slide_mask]
        coords_topleft = np.array(coords_topleft).astype(int)
        ...
        return patcher, center_coords
    
    def dump_all_cell_patches(
        self,
        patch_save_dir: str,
        name: str = 'patches',
        target_patch_size: int=224,
        ...
        patch_size_src = target_patch_size * (dst_pixel_size / src_pixel_size)
        coords_topleft = np.array(centroid_list) - patch_size_src // 2
        in_slide_mask = (0 <= coords_topleft[:, 0] + patch_size_src) & (coords_topleft[:, 0] < self.wsi.width) & (0 <= coords_topleft[:, 1] + patch_size_src) & (coords_topleft[:, 1] < self.wsi.height)
        coords_topleft = coords_topleft[in_slide_mask]
        coords_topleft = np.array(coords_topleft).astype(int)
        mask = self.tissue_contours if use_mask else None
